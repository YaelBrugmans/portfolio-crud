    ...
    <header>
        <nav>
            <ul class="nav first-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="?page=index&action=list">Accueil</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="?page=presentation&action=list">Présentation</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="?page=list&action=list">Travaux Réalisés</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="?page=services&action=list">Services</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="?page=contact&action=list">Contact</a>
                </li>
                <li class="nav-item">
...
function isFormSubmit() : bool {
    return isset($_POST['date']); // Si on a au moins un POST['date'] c'est que normalement le formulaire a été posté.
}
...
function uploadImage() {
    $filePath = './images/' . uniqid() . basename($_FILES['image']['name']);
    ...
    if(isFormSubmit()) {
        ...
        if (isFormValid()) {
            ...
            if($table == 'index'){
                ...
                $request = $db->prepare('INSERT INTO ' . $table . ' (`mail_expediteur`, `mail_destinataire`, `objet`, `contact_message`, `contact_date`) VALUES (:mail_expediteur, :mail_destinataire, :objet, :contact_message, :contact_date)');
                $params = [
...
function deletedataLine($db, $table){
    ...
    if (!isset($_GET['id'])) {
        ...
        if($table == 'index' || $table == 'presentation' || $table == 'travaux' || $table == 'services' || $table == 'contact'){
            ...
            $content = 'Un id est nécessaire pour supprimer ces données de la base de donnée. Veuillez <a href="/portfolio_crud/index.php?table=index&action=list">retourner à la liste index</a>';
        }
